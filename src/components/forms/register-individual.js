...
const validateForm = (values) => {
	const errors = {};
	if (!values.field_youth_group && !values.field_no_youth_group) {
		errors.field_youth_group = "You must choose a youth group to register for TYC";
	}
	return errors;
};
...
const RegisterIndividualForm = ({
	...
	return (
		<form onSubmit={handleSubmit}>
			...
			{!noYouthGroup && (
				<Field
					id="field_youth_group"
					name="field_youth_group"
					label="Your Youth Group"
					placeholder=""
					component={registerDropdown}
					options={mappedGroups}
					validate={[val.minLength(5), val.maxLength(50)]}
				/>
			)}
			{!youthGroup && (
				<Field
					id="field_no_youth_group"
					name="field_no_youth_group"
					label="My youth group is not coming to TYC or I'm not a part of a youth group"
					placeholder=""
					component={registerCheckbox}
				/>
			)}
			{noYouthGroup && (
				<p>
					We'd still love to have you at TYC. Please contact us on Facebook or at tasyouthcon@gmail.com and we'll sort
					out a youth group for you to come to TYC with.
				</p>
			)}
...
const IndividualReduxForm = reduxForm({
