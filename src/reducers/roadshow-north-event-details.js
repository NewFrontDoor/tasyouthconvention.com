...
export default (state, action) => {
  switch (action.type) {
    case types.REQUESTING_ROADSHOW_NORTH_EVENT_DETAILS:
      return Object.assign({}, state, { loading: true });
    case types.RECEIVED_ROADSHOW_NORTH_EVENT_DETAILS:
      return Object.assign({}, state, { loading: false, details: normaliseEventDetails(action.details) });
    case types.RECEIVED_ROADSHOW_NORTH_GROUP_INFORMATION:
...
export const areRegistrationsOpen = (state) => {
  const eventDetails = state.roadshowNorthEventDetails.details;
...
export const getGroups = (state) => state.roadshowNorthEventDetails.registeredGroups;
