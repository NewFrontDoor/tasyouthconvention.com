...
const savingRegistration = () => ({
  type: types.SAVING_ROADSHOW_SOUTH_GROUP_REGISTRATION
});

const errorSavingRegistration = (error) => ({
  type: types.ERROR_SAVING_ROADSHOW_SOUTH_GROUP_REGISTRATION,
  error
});

const savedRegistration = (details) => ({
  type: types.SAVED_ROADSHOW_SOUTH_GROUP_REGISTRATION,
  details
});

const savingPaymentDetails = () => ({
  type: types.SAVING_ROADSHOW_SOUTH_LEADER_PAYMENT_DETAILS
});

const errorSavingPaymentDetails = (error) => ({
  type: types.ERROR_SAVING_ROADSHOW_SOUTH_LEADER_PAYMENT_DETAILS,
  error
});

const savedPaymentDetails = (details) => ({
  type: types.SAVED_ROADSHOW_SOUTH_LEADER_PAYMENT_DETAILS
});

export const resetGroupRegistration = () => ({
  type: types.RESET_ROADSHOW_SOUTH_GROUP_REGISTRATION
});
